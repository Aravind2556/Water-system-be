...
AuthRouter.get('/fetch-User', async (req,res)=>{
    try{
        const fetch_User = await UserModel.find({})
        if(fetch_User){
            return res.send({success : true , user : fetch_User})
        }
        else{
            return res.send({success : false , message :"user data not avalible"})
        }
